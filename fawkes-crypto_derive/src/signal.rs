...
use quote::quote;
use syn::{Data, DeriveInput, Field, Fields, Ident, Type, parse_str, PathSegment, Path, FieldsUnnamed, FieldsNamed};
...
pub fn expand(input: &DeriveInput, _: &str) -> TokenStream {
    ...
    let value_type = parse_str::<Type>(&fetch_attr("Value", &input.attrs).expect("attribute value should be defined")).expect("attribute should be a type");
...
fn get_typename(t:&Type) -> &Ident {
    if let Type::Path(t) = t {
        let Path {leading_colon: _, segments: t} = &t.path;
        let PathSegment{ident: i, arguments: _} = &t[0];
        i
    } else {
...
fn tuple_impl(fields: &[&Field]) -> TokenStream {
    ...
    let var_ids = (0..fields.len()).map(|i| syn::Index::from(i)).collect::<Vec<_>>();
