...
use ff_uint::BitIterLE;
use crate::circuit::{num::CNum, bool::CBool};
use crate::core::signal::Signal;



pub fn c_into_bits_le<Fr: PrimeField>(
    signal:&CNum<Fr>,
    limit: usize
) -> Vec<CBool<Fr>>
{
...
pub fn c_comp_constant<Fr: PrimeField>(
    signal:&[CBool<Fr>],
    ct: Num<Fr>
) -> CBool<Fr> {
    ...
    let sig_zero = if siglen & 1 == 1 {vec![CBool::from_const(cs, &false)] } else {vec![]};
...
pub fn c_into_bits_le_strict<Fr: PrimeField>(
    signal:&CNum<Fr>
) -> Vec<CBool<Fr>>{
...
pub fn c_from_bits_le<Fr: PrimeField>(
    bits:&[CBool<Fr>]
) -> CNum<Fr> {
