        ...
        loop {
            ...
            for j in i..t.unwrap_or(vars_length) {
                let v = bellman_cs.alloc(
                    || format!("var_{}", j), 
                    || cs.values[j].map(|v| num_to_bellman_fp(v) ).ok_or(SynthesisError::AssignmentMissing)
                ).unwrap();
                variables.push(v);
            }
            ...
            match t {
                Some(t) => {
                    let v = bellman_cs.alloc_input(
                        || format!("var_{}", t), 
                        || cs.values[t].map(|v| num_to_bellman_fp(v) ).ok_or(SynthesisError::AssignmentMissing)
                    ).unwrap();
                    variables.push(v); 
                    i=t+1;
                }
