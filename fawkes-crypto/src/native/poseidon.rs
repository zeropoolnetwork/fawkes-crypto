...
use ff_uint::seedbox::SeedboxBlake2;
...
impl<Fr:PrimeField> PoseidonParams<Fr> {
    pub fn new(t:usize, f:usize, p:usize) -> Self {
        ...
        let c = (0..f+p).map(|_| (0..t).map(|_| seedbox.gen()).collect()).collect();
...
pub fn poseidon_with_salt<Fr:PrimeField>(inputs:&[Num<Fr>], seed: &[u8], params:&PoseidonParams<Fr>) -> Num<Fr> {
    let n_inputs = inputs.len();
    assert!(n_inputs > 0, "number of inputs should be positive nonzero");
...
pub fn poseidon_merkle_proof_root<Fr:PrimeField, L:Unsigned>(leaf:Num<Fr>, proof:&MerkleProof<Fr, L>, params:&PoseidonParams<Fr>) -> Num<Fr> {
...
pub fn poseidon_merkle_tree_root<Fr:PrimeField>(leaf:&[Num<Fr>], params: &PoseidonParams<Fr>) -> Num<Fr> {
