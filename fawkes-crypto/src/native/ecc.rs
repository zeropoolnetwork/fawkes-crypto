...
use rand::Rng;
use ff_uint::{Num, PrimeField, BitIterBE};
...
impl <Fr: PrimeField> EdwardsPoint<Fr> {
    ...
    pub fn subgroup_decompress<J: JubJubParams<Fr=Fr>>(x: Num<Fr>, params: &J) -> Option<Self>
    {
        ...
        match t {
            Some(y) => {
                let EdwardsPoint {x:lx, y:ly} = EdwardsPoint{x, y}.into_extended().mul(Num::<J::Fs>::MODULUS, params).into_affine();
                if lx.is_zero() {
                    if ly == Num::ONE {
                        Some(Self{x, y})
                    } else {
    ...
    pub fn from_scalar_raw(t:Num<Fr>, montgomery_a:Num<Fr>, montgomery_b:Num<Fr>, montgomery_u:Num<Fr>) -> Self {
...
impl <Fr: PrimeField> EdwardsPointEx<Fr> {
    ...
    pub fn mul<S: BitIterBE, J: JubJubParams<Fr=Fr>>(
        &self,
        scalar: S,
        params: &J
    ) -> Self
