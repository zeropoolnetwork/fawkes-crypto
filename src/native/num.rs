...
impl<T: Field> BorshDeserialize for Num<T> {
    fn deserialize(buf: &mut &[u8]) -> Result<Self, io::Error> {
        ...
        repr.read_le(buf)?;
        Ok(Num(T::from_repr(repr).map_err(|_| {
            io::Error::new(ErrorKind::InvalidData, "Value should be in field")
        })?))
    }
}
...
impl<'de, T: Field> Deserialize<'de> for Num<T> {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Num<T>, D::Error> {
        let bn = BigUint::from_str(&<String as Deserialize>::deserialize(deserializer)?)
            .map_err(|_| de::Error::custom("Wrong number format"))?;
...
mod num_test {
    ...
    fn num_test() {
        ...
        assert!(
            Into::<BigUint>::into(a + b)
                == (Into::<BigUint>::into(a) + Into::<BigUint>::into(b)) % &order
        );
        assert!(
            Into::<BigUint>::into(a * b)
                == (Into::<BigUint>::into(a) * Into::<BigUint>::into(b)) % &order
        ...
        assert!(
            Into::<BigUint>::into(a - b)
                == (&order + Into::<BigUint>::into(a) - Into::<BigUint>::into(b)) % &order
        );
        assert!(
            BigUint::from(1u64)
                == (Into::<BigUint>::into(a.inverse()) * Into::<BigUint>::into(a)) % &order
        );
    }
}
