        ...
        mod $modname {
            ...
            use $crate::{
                ...
                helpers::groth16::{
                    ...
                    prover::{generate_keys, prove, Parameters, Proof},
                    verifier::{
                        truncate_verifying_key, verify, TruncatedVerifyingKey,
            ...
            impl fawkes_crypto::core::cs::Circuit for CircuitObject {
                type F = Fr;
                fn synthesize<
                    CS: fawkes_crypto::core::cs::ConstraintSystem<F = pairing::bn256::Fr>,
                >(
                    &self,
                    cs: &CS,
                ) {
            ...
            fn cli_setup(o: SetupOpts) {
                ...
                let vk_data_str =
                    serde_json::to_string_pretty(&truncate_verifying_key(&params.vk).into_data())
                        .unwrap();
            ...
            fn cli_verify(o: VerifyOpts) {
                ...
                let vk = TruncatedVerifyingKey::<Bn256>::from_data(
                    &serde_json::from_str(&vk_str).unwrap(),
                );
                let proof = Proof::<Bn256>::from_data(&serde_json::from_str(&proof_str).unwrap());
                let public_inputs = serde_json::from_str::<Vec<Num<Fr>>>(&public_inputs_str)
                    .unwrap()
                    .into_iter()
                    .map(|e| e.into_inner())
                    .collect::<Vec<_>>();
                ...
                println!(
                    "Verify result is {}.",
                    verify(&vk, &proof, &public_inputs).unwrap_or(false)
                )
            }
            ...
            fn cli_prove(o: ProveOpts) {
                let params =
                    Parameters::<Bn256>::read(File::open(o.params).unwrap(), false).unwrap();
        ...
        mod $modname {
            ...
            pub fn num_vec_to_buf<Fr: Field>(nums: &[Num<Fr>]) -> Vec<u8> {
                ...
                for (i, e) in nums.iter().enumerate() {
                    e.0.into_repr()
                        .write_be(&mut data[fr_repr_sz * i..])
                        .unwrap();
                }
                data
            }
            ...
            impl fawkes_crypto::core::cs::Circuit for CircuitObject {
                type F = Fr;
                fn synthesize<
                    CS: fawkes_crypto::core::cs::ConstraintSystem<F = pairing::bls12_381::Fr>,
                >(
                    &self,
                    cs: &CS,
                ) {
            ...
            fn cli_setup(o: SetupOpts) {
                ...
                truncate_verifying_key(&params.vk)
                    .write(&mut tvk_data)
                    .unwrap();
            ...
            fn cli_verify(o: VerifyOpts) {
                ...
                let vk =
                    TruncatedVerifyingKey::<Bls12>::read(&base64::decode(&vk_str).unwrap()[..])
                        .unwrap();
                let proof = Proof::<Bls12>::read(&base64::decode(&proof_str).unwrap()[..]).unwrap();
                let public_inputs = read_num_vec(&base64::decode(&public_inputs_str).unwrap()[..])
                    .unwrap()
                    .iter()
                    .map(|n| n.0)
                    .collect::<Vec<_>>();
            ...
            fn cli_prove(o: ProveOpts) {
                let params =
                    Parameters::<Bls12>::read(File::open(o.params).unwrap(), false).unwrap();
                let object_str = std::fs::read_to_string(o.object).unwrap();
                ...
                std::fs::write(o.proof, &base64::encode(proof_data.get_ref()).into_bytes())
                    .unwrap();
                std::fs::write(
                    o.inputs,
                    &base64::encode(&num_vec_to_buf(&inputs)).into_bytes(),
                )
                .unwrap();
                println!("Proved");
            }
    ...
    ($modname:ident, $cir_pub:ty, $ccir_pub:ident, $cir_sec:ty, $ccir_sec:ident, $cir_params:ident, $cir_main:ident, $test_data:ident) => {
        mod $modname {
            ...
            fn cli_verify(o: VerifyOpts) {
                ...
                let vk =
                    TruncatedVerifyingKey::<Bn256>::read(&base64::decode(&vk_str).unwrap()[..])
                        .unwrap();
    ...
    ($modname:ident, $cir_pub:ty, $ccir_pub:ident, $cir_sec:ty, $ccir_sec:ident, $cir_params:ident, $cir_main:ident, $test_data:ident) => {
        mod $modname {
            ...
            fn cli_verify(o: VerifyOpts) {
                ...
                let vk = TruncatedVerifyingKey::<Bn256>::from_data(
                    &<_>::try_from_slice(&base64::decode(&vk_str).unwrap()[..]).unwrap(),
                );
                let proof = Proof::<Bn256>::from_data(
                    &<_>::try_from_slice(&base64::decode(&proof_str).unwrap()[..]).unwrap(),
                );
                let public_inputs = <Vec<Num<Fr>>>::try_from_slice(
                    &base64::decode(&public_inputs_str).unwrap()[..],
                )
                .unwrap()
                .iter()
                .map(|n| n.0)
