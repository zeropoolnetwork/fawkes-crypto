...
lazy_static! {
    static ref BLS12_381_B_COEFF: bls12_381::Fq =
        bls12_381::Fq::from_raw_repr(bls12_381::FqRepr([
            0xaa270000000cfff3,
            0x53cc0032fc34000a,
            0x478fe97a6b0a807f,
            0xb1d37ebee6ba24d7,
...
impl Into<bn256::G2Affine> for G2PointData<bn256::Fq> {
    fn into(self) -> bn256::G2Affine {
        //use big endian here
        let x = bn256::Fq2 {
            c0: self.0 .1.into_inner(),
            c1: self.0 .0.into_inner(),
        };
        let y = bn256::Fq2 {
            c0: self.1 .1.into_inner(),
            c1: self.1 .0.into_inner(),
        };
        let is_zero = x.is_zero() && y.is_zero();
        assert!(
            is_on_curve(x, y, is_zero, &BN256_B_COEFF_FQ2),
            "point should be in curve"
        );
...
impl Into<bn256::G1Affine> for G1PointData<bn256::Fq> {
    fn into(self) -> bn256::G1Affine {
        ...
        let is_zero = x.is_zero() && y.is_zero();
        assert!(
            is_on_curve(x, y, is_zero, &BN256_B_COEFF),
            "point should be in curve"
        );
...
impl Into<bls12_381::G2Affine> for G2PointData<bls12_381::Fq> {
    fn into(self) -> bls12_381::G2Affine {
        ...
        let is_zero = x.is_zero() && y.is_zero();
        assert!(
            is_on_curve(x, y, is_zero, &BLS12_381_B_COEFF_FQ2),
            "point should be in curve"
        );
        let res = unsafe { transmute::<_, bls12_381::G2Affine>((x, y, is_zero)) };
        assert!(
            res.mul(bls12_381::Fr::char()).is_zero(),
...
impl Into<bls12_381::G1Affine> for G1PointData<bls12_381::Fq> {
    fn into(self) -> bls12_381::G1Affine {
        ...
        let is_zero = x.is_zero() && y.is_zero();
        assert!(
            is_on_curve(x, y, is_zero, &BLS12_381_B_COEFF),
            "point should be in curve"
        );
...
impl<BE: bellman::pairing::Engine, BCS: bellman::ConstraintSystem<BE>> Clone
    for Groth16CS<BE, BCS>
{
    fn clone(&self) -> Self {
...
impl<BE: bellman::pairing::Engine, BCS: bellman::ConstraintSystem<BE>> ConstraintSystem
    for Groth16CS<BE, BCS>
{
    type F = BE::Fr;
    ...
    fn alloc(&self, value: Option<Num<Self::F>>) -> Index {
        ...
        *naux_ref += 1;
        self.bcs
            .borrow_mut()
            .alloc(|| format!("a[{}]", naux), || value.grab())
    ...
    fn alloc_input(&self, value: Option<Num<Self::F>>) -> Index {
        ...
        *ninputs_ref += 1;
        self.bcs
            .borrow_mut()
            .alloc_input(|| format!("i[{}]", ninputs), || value.grab())
            .map(|e| unsafe { std::mem::transmute(e) })
            .unwrap()
    }

    fn enforce(&self, a: &CNum<Self>, b: &CNum<Self>, c: &CNum<Self>) {
        fn into_bellman_lc<BE: bellman::pairing::Engine, CS: ConstraintSystem>(
            s: &CNum<CS>,
        ) -> bellman::LinearCombination<BE> {
            let res =
                s.lc.iter()
        ...
        *ncons_ref += 1;
        ...
        self.bcs
            .borrow_mut()
            .enforce(|| format!("c[{}]", ncons), |_| a, |_| b, |_| c);
    }
}
