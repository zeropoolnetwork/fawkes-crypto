...
macro_rules! forward_all_binop_to_val_ref_ex {
    (impl<$($imp_l:lifetime, )*$($imp_i:ident : $imp_p:path),+> $imp:ident for $res:ty, $method:ident) => {
        forward_val_val_binop_ex!(impl<$($imp_l, )*$($imp_i : $imp_p),+> $imp for $res, $method);
        forward_ref_val_binop_ex!(impl<$($imp_l, )*$($imp_i : $imp_p),+> $imp for $res, $method);
        forward_ref_ref_binop_ex!(impl<$($imp_l, )*$($imp_i : $imp_p),+> $imp for $res, $method);
    };
}
