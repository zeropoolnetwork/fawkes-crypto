use crate::{
    circuit::bool::CBool, circuit::num::CNum, core::cs::ConstraintSystem, native::num::Num,
};
...
pub fn c_mux3<'a, CS: ConstraintSystem>(
    s: &[CBool<'a, CS>],
    c: &[Vec<Num<CS::F>>],
) -> Vec<CNum<'a, CS>> {
    ...
    for i in 0..c.len() {
        ...
        let a21 = (c[i][6] - c[i][4] - c[i][2] + c[i][0]) * &s[1].0;
        let a20 = (c[i][5] - c[i][4] - c[i][1] + c[i][0]) * &s[0].0;
        let a2 = c[i][4] - c[i][0];

        let a10 = (c[i][3] - c[i][2] - c[i][1] + c[i][0]) * &s10;
        let a1 = (c[i][2] - c[i][0]) * &s[1].0;
        let a0 = (c[i][1] - c[i][0]) * &s[0].0;
        let a = c[i][0];

        res.push((a210 + a21 + a20 + a2) * &s[2].0 + a10 + a1 + a0 + a);
    }
    res
}
