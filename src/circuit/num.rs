...
use crate::{
    circuit::bool::CBool, core::cs::ConstraintSystem, core::signal::Signal, native::num::Num,
};
...
impl<'a, CS: ConstraintSystem> Signal<'a, CS> for CNum<'a, CS> {
    ...
    fn switch(&self, bit: &CBool<'a, CS>, if_else: &Self) -> Self {
        match bit.as_const() {
            ...
            _ => {
                if if_else.capacity() < self.capacity() {
    ...
    fn assert_const(&self, c: &Self::Value) {
        match self.as_const() {
            ...
            _ => {
                self.cs
                    .enforce(self, &self.derive_one(), &self.derive_const(c));
            }
        }
    }
...
impl<'a, CS: ConstraintSystem> CNum<'a, CS> {
    ...
    pub fn assert_bit(&self) {
        match self.as_const() {
            ...
            _ => {
                self.cs
                    .enforce(self, &(self - Num::one()), &self.derive_zero());
            }
        }
    }
